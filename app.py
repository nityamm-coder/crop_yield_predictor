...
indian_state_rainfall = {
    ...
    'Maharashtra': 1200, 'Odisha': 1500, 'Punjab': 600,
    'Rajasthan': 600, 'Tamil Nadu': 1000, 'Telangana': 900,
...
indian_state_temperature = {
    ...
    'Maharashtra': 26, 'Odisha': 27, 'Punjab': 23,
    'Rajasthan': 27, 'Tamil Nadu': 28, 'Telangana': 28,
    'Uttar Pradesh': 25, 'Uttarakhand': 20, 'West Bengal': 26
}
...
def get_crop_factor(crop, temp, rain):
    ...
    tf = 1.0 if c["temp_min"] <= temp <= c["temp_max"] else 0.8
    rf = 1.0 if c["rain_min"] <= rain <= c["rain_max"] else 0.8
    return tf * rf
...
def index():
    ...
    if request.method == "POST":
        ...
        pred = model.predict(np.hstack([[encoded_crop], scaled[0]]).reshape(1, -1))
        result = yield_encoder.inverse_transform(pred)[0]

        conditions = crop_optimal_conditions.get(crop, crop_optimal_conditions["DEFAULT"])
        impacts = []
        impacts.append("✓ Optimal temperature" if conditions["temp_min"] <= temperature <= conditions["temp_max"]
                       else "⚠ Temperature not optimal")
        impacts.append("✓ Optimal rainfall" if conditions["rain_min"] <= rainfall <= conditions["rain_max"]
                       else "⚠ Rainfall not optimal")

        context.update({
            "result": result,
            "estimated_yield": round(adjusted_yield, 2),
